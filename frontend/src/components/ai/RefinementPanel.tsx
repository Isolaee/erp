...
export function RefinementPanel({ open, onClose, targetType, targetId }: Props) {
  ...
  const handleRefine = async () => {
    ...
    const response = await fetch(`${base}/api/ai/refine`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
      },
      body: JSON.stringify({ targetType, targetId, prompt: prompt.trim() }),
    });
    ...
    abortRef.current = () => { reader.cancel(); setStreaming(false); };
    ...
    const parse = (chunk: string): StreamEvent[] => {
      ...
      for (const line of lines) {
        if (line.startsWith('event: ')) { eventType = line.slice(7).trim(); }
          ...
          try { events.push({ type: eventType as any, data: JSON.parse(dataStr) }); } catch { /* skip */ }
          eventType = ''; dataStr = '';
        }
      }
      return events;
        ...
        for (const part of parts) {
          for (const evt of parse(part + '\n\n')) {
            if (evt.type === 'text') {
              ...
              queryClient.invalidateQueries({ queryKey: ['lists', targetId] });
    ...
    <div className="fixed inset-y-0 right-0 z-40 w-96 bg-white shadow-xl flex flex-col border-l border-gray-200">
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900">AI Refine</h2>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {!text && !streaming && (
          <p className="text-sm text-gray-400 text-center pt-8">
            Ask AI to refine this {targetType}. It can create, update, move, and assign tasks.
          </p>
        )}
      </div>
      ...
      <div className="p-4 border-t border-gray-200">
        <textarea
          ...
          rows={3}
          placeholder="e.g. Break this into 3 subtasks, assign the backend work to someone with highest workload..."
        ...
        >
          {streaming ? <><Spinner className="h-4 w-4" /> Refining...</> : <><Sparkles className="h-4 w-4" /> Refine</>}
        </button>
